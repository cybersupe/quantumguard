...
function Scanner({ darkMode }) {
  ...
  const handleScan = async () => {
    setLoading(true); setProgress(0); setError(null); setResult(null); setChecklist({});
            ...
            {filteredFindings.map((f, i) => {
              ...
              return (
                <div key={i} style={{ borderLeft: `3px solid ${f.severity === "CRITICAL" ? "#E24B4A" : f.severity === "HIGH" ? "#BA7517" : "#1D9E75"}`, paddingLeft: 16, marginBottom: 16, opacity: checklist[key] ? 0.5 : 1 }}>
                  ...
                  <div style={{ fontFamily: "monospace", background: bg, padding: "8px 12px", borderRadius: 6, fontSize: 13, marginBottom: 4, color: text, overflowX: "auto" }}>{f.code}</div>
