...
function Navbar({ darkMode, setDarkMode }) {
  return (
    <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 40px", borderBottom: `1px solid ${darkMode ? "#1a1a2e" : "#e5e5e5"}`, position: "sticky", top: 0, background: darkMode ? "#0f0f1a" : "#ffffff", zIndex: 100 }}>
      <div style={{ fontSize: 20, fontWeight: 700, color: "#7F77DD" }}>⚛ QuantumGuard</div>
      <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
        <a href="#how" style={{ color: darkMode ? "#888" : "#666", textDecoration: "none", fontSize: 14 }}>How it works</a>
        <a href="#pricing" style={{ color: darkMode ? "#888" : "#666", textDecoration: "none", fontSize: 14 }}>Pricing</a>
        <a href="https://github.com/cybersupe/quantumguard" target="_blank" rel="noreferrer" style={{ color: darkMode ? "#888" : "#666", textDecoration: "none", fontSize: 14 }}>GitHub</a>
        <button onClick={() => setDarkMode(!darkMode)} style={{ background: "transparent", border: `1px solid ${darkMode ? "#333" : "#ddd"}`, borderRadius: 8, padding: "6px 12px", cursor: "pointer", color: darkMode ? "#888" : "#666", fontSize: 13 }}>
...
function Scanner({ darkMode }) {
  ...
  const detectedLanguages = result ? [...new Set(result.findings.map(f => {
    const ext = f.file.split('.').pop();
    const map = { py: 'Python', js: 'JavaScript', java: 'Java', ts: 'TypeScript' };
    return map[ext] || ext;
  }))] : [];
  ...
  const handleShare = () => {
    if (!result) return;
    const text = `I just scanned my codebase with QuantumGuard!\n\nQuantum Readiness Score: ${result.quantum_readiness_score}/100\nVulnerabilities Found: ${result.total_findings}\n\nCheck your code at quantumguard-one.vercel.app\n\n#QuantumSecurity #CyberSecurity #OpenSource`;
  ...
  return (
    <div id="scan" style={{ maxWidth: 860, margin: "0 auto", padding: "60px 20px", background: bg }}>
      ...
      <p style={{ color: muted, textAlign: "center", marginBottom: 32, fontSize: 14 }}>Upload a ZIP file or use server path</p>
        ...
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 16 }}>
            {[
              ...
              { key: "HIGH", color: "#BA7517" },
              { key: "MEDIUM", color: "#1D9E75" },
            ].map(s => (
          ...
          {detectedLanguages.length > 0 && (
            <div style={{ background: card, borderRadius: 8, padding: "12px 16px", marginBottom: 16, border: `1px solid ${border}` }}>
              <span style={{ fontSize: 13, color: muted, marginRight: 8 }}>Languages detected:</span>
              {detectedLanguages.map((l, i) => (
                <span key={i} style={{ background: "#534AB722", color: "#7F77DD", padding: "2px 10px", borderRadius: 20, fontSize: 12, marginRight: 6 }}>{l}</span>
              ))}
            </div>
          )}

          {/* File breakdown */}
          {fileBreakdown && Object.keys(fileBreakdown).length > 0 && (
            <div style={{ background: card, borderRadius: 8, padding: "16px", marginBottom: 16, border: `1px solid ${border}` }}>
          ...
          <div style={{ background: card, borderRadius: 12, padding: 24, border: `1px solid ${border}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={handleCopy} style={{ padding: "6px 14px", borderRadius: 6, background: copied ? "#1D9E75" : "transparent", color: copied ? "#fff" : muted, border: `1px solid ${border}`, cursor: "pointer", fontSize: 12 }}>{copied ? "Copied!" : "Copy Report"}</button>
                ...
                <button onClick={handleShare} style={{ padding: "6px 14px", borderRadius: 6, background: "#1DA1F2", color: "#fff", border: "none", cursor: "pointer", fontSize: 12 }}>Share on X</button>
              </div>
            </div>

            <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
              {["ALL", "CRITICAL", "HIGH", "MEDIUM"].map(f => (
                <button key={f} onClick={() => setFilter(f)} style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid ${border}`, background: filter === f ? "#534AB7" : "transparent", color: filter === f ? "#fff" : muted, cursor: "pointer", fontSize: 12 }}>{f} {f !== "ALL" && severityCounts ? `(${severityCounts[f]})` : ""}</button>
              ))}
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search files..." style={{ padding: "4px 12px", borderRadius: 20, border: `1px solid ${border}`, background: bg, color: text, fontSize: 12, width: 150 }} />
            </div>
              ...
              return (
                <div key={i} style={{ borderLeft: `3px solid ${f.severity === "CRITICAL" ? "#E24B4A" : f.severity === "HIGH" ? "#BA7517" : "#1D9E75"}`, paddingLeft: 16, marginBottom: 16, opacity: checklist[key] ? 0.5 : 1 }}>
                  <div style={{ display: "flex", gap: 8, marginBottom: 4, alignItems: "center" }}>
                    <input type="checkbox" checked={!!checklist[key]} onChange={() => toggleChecklist(key)} style={{ cursor: "pointer" }} />
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, color: text, fontFamily: "sans-serif" }}>
      ...
      <div style={{ textAlign: "center", padding: "80px 20px 60px" }}>
        ...
        <h1 style={{ fontSize: 52, fontWeight: 700, lineHeight: 1.2, marginBottom: 20, maxWidth: 700, margin: "0 auto 20px", color: text }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
          ...
          <a href="https://github.com/cybersupe/quantumguard" target="_blank" rel="noreferrer" style={{ background: card, color: text, padding: "14px 32px", borderRadius: 10, textDecoration: "none", fontSize: 15, border: `1px solid ${border}` }}>View on GitHub</a>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, maxWidth: 800, margin: "0 auto 80px", padding: "0 20px" }}>
        {[{ num: "6+", label: "Algorithms Detected" }, { num: "4", label: "Languages Supported" }, { num: "2030", label: "Quantum Deadline" }, { num: "100%", label: "Open Source" }].map((s, i) => (
          <div key={i} style={{ background: card, borderRadius: 12, padding: 20, textAlign: "center", border: `1px solid ${border}` }}>
      ...
      <div id="how" style={{ maxWidth: 700, margin: "0 auto 80px", padding: "0 20px" }}>
        <h2 style={{ textAlign: "center", fontSize: 32, marginBottom: 8, color: text }}>How it works</h2>
        <p style={{ textAlign: "center", color: muted, marginBottom: 48, fontSize: 15 }}>Five simple steps to quantum-proof your code</p>
        <div style={{ position: "relative" }}>
          ...
          {[
            ...
            { step: "4", icon: "📊", title: "Review Findings", desc: "Filter by CRITICAL, HIGH or MEDIUM. Search specific files. See your Quantum Readiness Score from 0 to 100." },
            { step: "5", icon: "📄", title: "Download Your Report", desc: "Download a PDF report or copy findings. Share on Twitter or present to your board." },
          ].map((s, i) => (
      ...
      <div style={{ maxWidth: 900, margin: "0 auto 80px", padding: "0 20px" }}>
        <h2 style={{ textAlign: "center", fontSize: 32, marginBottom: 8, color: text }}>What We Detect</h2>
        <p style={{ textAlign: "center", color: muted, marginBottom: 40, fontSize: 15 }}>All algorithms flagged by NIST as quantum-vulnerable</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {[
            ...
            <div key={i} style={{ background: card, borderRadius: 12, padding: 16, borderLeft: `3px solid ${v.severity === "CRITICAL" ? "#E24B4A" : v.severity === "HIGH" ? "#BA7517" : "#1D9E75"}`, border: `1px solid ${border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <span style={{ fontWeight: 600, fontSize: 15, color: text }}>{v.name}</span>
              ...
              <div style={{ fontSize: 12, color: muted }}>Fix: <span style={{ color: "#7F77DD" }}>{v.fix}</span></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div id="pricing" style={{ maxWidth: 900, margin: "0 auto 80px", padding: "0 20px" }}>
        <h2 style={{ textAlign: "center", fontSize: 32, marginBottom: 8, color: text }}>Pricing</h2>
        <p style={{ textAlign: "center", color: muted, marginBottom: 40, fontSize: 15 }}>Start free, upgrade when you need more</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {[
            ...
            <div key={i} style={{ background: card, borderRadius: 16, padding: 28, border: p.highlight ? "2px solid #534AB7" : `1px solid ${border}`, position: "relative" }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 4, color: text }}>{p.name}</div>
              <div style={{ fontSize: 28, fontWeight: 700, color: "#7F77DD", marginBottom: 4 }}>{p.price}</div>
              <div style={{ fontSize: 13, color: muted, marginBottom: 20 }}>{p.desc}</div>
              {p.features.map((f, j) => <div key={j} style={{ fontSize: 13, color: muted, marginBottom: 8 }}>✓ {f}</div>)}
              <button style={{ width: "100%", marginTop: 20, padding: "10px", borderRadius: 8, background: p.highlight ? "#534AB7" : "transparent", color: p.highlight ? "#fff" : "#534AB7", border: `1px solid #534AB7`, cursor: "pointer", fontSize: 14, fontWeight: 500 }}>{p.cta}</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 700, margin: "0 auto 80px", padding: "0 20px" }}>
        <h2 style={{ textAlign: "center", fontSize: 32, marginBottom: 40, color: text }}>FAQ</h2>
        {[
          ...
          <div key={i} style={{ background: card, borderRadius: 12, padding: 20, marginBottom: 12, border: `1px solid ${border}` }}>
            <div style={{ fontWeight: 600, marginBottom: 8, fontSize: 15, color: text }}>{f.q}</div>
            <div style={{ color: muted, fontSize: 14, lineHeight: 1.6 }}>{f.a}</div>
          </div>
        ))}
      </div>

      {/* CTA */}
      <div style={{ textAlign: "center", padding: "60px 20px", background: card }}>
        <h2 style={{ fontSize: 32, marginBottom: 16, color: text }}>Ready to quantum-proof your code?</h2>
        <p style={{ color: muted, marginBottom: 32, fontSize: 15 }}>Join developers and security teams protecting their code from quantum threats.</p>
      ...
      {/* Footer */}
      <div style={{ textAlign: "center", padding: "24px 20px", borderTop: `1px solid ${border}`, color: muted, fontSize: 13 }}>
        <div style={{ marginBottom: 8 }}>
          <a href="https://github.com/cybersupe/quantumguard" target="_blank" rel="noreferrer" style={{ color: muted, textDecoration: "none", marginRight: 20 }}>GitHub</a>
          <a href="#how" style={{ color: muted, textDecoration: "none", marginRight: 20 }}>How it works</a>
